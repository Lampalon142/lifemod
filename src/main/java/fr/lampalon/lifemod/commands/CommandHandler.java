    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (plugin.isLifemodActive()) {
            ...
            if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
                if (sender.hasPermission("lifemod.reload")) {
                    plugin.reloadConfig();
                    plugin.ConfigConfig();
                ...
                sender.sendMessage(MessageUtil.parseColors(plugin.getConfigConfig().getString("prefix") + plugin.getConfigConfig().getString("config.usage")));
            }
        } else {
